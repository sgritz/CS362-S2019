/****************************************************************
 * Assignment 4 - randomtest1 - Smithy (+3 cards)
...
int main(){
	...
	for (i = 0; i <500; i++){		//test with some randomized conditions 500 times
		...
		smithyCardEffect(&game, handpos) ;
		
		deckPostCount = game.deckCount[player];
		handPostCount = game.handCount[player];
		
		if (deckPreCount-deckPostCount != 3){	//deck count should decrease by 3
		...
		if (handPostCount-handPreCount != 3){	//hand count should increase by 3-1
