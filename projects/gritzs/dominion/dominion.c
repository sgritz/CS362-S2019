...
//Refactoring 5 cardEffects as separate functions for Assignment 2
...
int playAdventurer(int currentPlayer, struct gameState *state){
	...
	int temphand[MAX_HAND];// moved 
	
	while(drawntreasure<=2){  //< changed to <= for *BUG*
...
int playFeast(int currentPlayer, struct gameState *state, int choice1){					//gain card with cost up to 5
	int temphand[MAX_HAND];// moved 
    ...
    int x = 0;//Condition to loop on   // x = 1 changed to x = 0 for *BUG*
...
int playCouncilRoom(int currentPlayer, struct gameState *state, int handPos){
	...
	//+4 Cards
    for (int i = 0; i < 3; i++){ // i < 4 changed to i < 3 for *BUG*
		drawCard(currentPlayer, state);
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case feast:
		return playFeast(currentPlayer, state, choice1);
